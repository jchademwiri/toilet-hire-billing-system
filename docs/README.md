...
into one printable set — each is also available at its own route (`/invoices/[id]`, `/service-notes/[id]`,
`/cleaning-schedule/[id]`, `/epwp/[id]`, `/coordinates/[id]`) via the document sidebar.
